# Data Layer Contract — SPECIFICATION v1.3 (QR-DATA-001)
...
2. **AUTHORIZED (DECISION 4, v1.3).** The compact streaming extraction path is approved and MUST be
   implemented. The pipeline is strictly incremental, one archive segment at a time:
...
8. ~~`asset_ctxs` not authorized~~ — **AUTHORIZED by DECISION 4** (D§5.5.1). `event_price` becomes
   available; `period_start` remains the fallback when the compact artifact is absent.
