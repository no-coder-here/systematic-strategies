# Experiment Registry — SPECIFICATION v1.3 (QR-INFRA-002 / QR-INFRA-002-A)

**Status: FROZEN — 2026-08-17, Research Lead.**
`schema_version = "qr-infra-002-v1.3"`.
**R§21 (QR-INFRA-002-A CLOSURE) is the newest governing section: R§21 > R§20 > R§1–R§19.**
...
| ver | date | cause |
|---|---|---|
| v1.3 | 2026-08-17 | **QR-INFRA-002-A targeted closure (R§21).** Scoped fix of the 4 production defects + 20 known surviving mutations from the final v1.2 audit. Architecture UNCHANGED and accepted as-is (two/three-hash identity, id derivation, append-only write-once plain files, chained history, registry-above-engine). Normative changes: adapter trust moves behind an unforgeable internal capability (`_recorded_via`/`_logged_at_override` public keywords REMOVED); `n_configs_evaluated` becomes REQUIRED tri-state (int ≥ 1 / `None` = UNKNOWN) so omission can no longer read as a verified 1; per-field identity protection mandated; `config_family_hash`'s exclusion of `frozen_spec_sha256` legitimised. |
...
## R§4 Record schema (`schema_version` = the constant in R§20.0/R§21; **v1.3** as of QR-INFRA-002-A)
...
# R§21 QR-INFRA-002-A — TARGETED CLOSURE

**Status: NORMATIVE AND GOVERNING. R§21 > R§20 > R§1–R§19.**

Scope is deliberately narrow: the 4 production defects and ~20 known surviving mutation behaviours
found by the final v1.2 audit, plus the `n_configs_evaluated` default. **The architecture is
accepted and MUST NOT be redesigned** — `semantic_hash`, `exact_hash`, `config_family_hash`,
`experiment_id = f(exact_hash, run_seq)`, append-only plain files, write-once records, chained
history, and registry-above-engine all stand as specified. No new architecture may be introduced
except where a listed defect cannot otherwise be fixed (R§21.1 is the one such case).

## R§21.1 Adapter trust becomes unforgeable (production defect 1 — blocking)

Measured on v1.2: `_recorded_via` and `_logged_at_override` were ordinary keyword arguments of the
public `record_experiment`, so a single keyword bought adapter trust —
`record_experiment(..., _recorded_via="adapter")` with a proxy dataset declared native produced
`recorded_via="adapter"`, **no** `UNVERIFIED_MANUAL_RESULTS`, no required
`manual_results_justification`, and a `summary()` reading `native` with no caveat line. Every
R§20.3 guard keys off `recorded_via`, so this reconstituted the entire proxy-as-native exploit.
`_logged_at_override=created_at` likewise suppressed `BACKDATED_CREATED_AT`.

- **R§21.1.1 (blocking).** `_recorded_via` and `_logged_at_override` are **REMOVED** from
  `record_experiment`'s signature. Neither may reappear in any form a public caller can express.
- **R§21.1.2 (blocking).** Adapter trust is conferred only by an internal capability:
  a module-private singleton in `store.py` (e.g. `_ADAPTER_CAPABILITY = _AdapterCapability()`),
  accepted as `record_experiment(..., capability=<obj>)` and validated by **identity** (`is`).
  - `capability is None` ⇒ `recorded_via = "manual"`.
  - `capability is _ADAPTER_CAPABILITY` ⇒ `recorded_via = "adapter"`.
  - **anything else ⇒ `RegistryError`.** A forged look-alike (a duck-typed object, a subclass, a
    string, a copy, `_AdapterCapability()` freshly constructed by a caller) MUST be rejected.
    Identity, not type, not truthiness — a `isinstance` check would be forgeable by subclassing.
  - Only `backtest_adapter.record_backtest_result` may pass it.
- **R§21.1.3 (blocking).** The wall-clock witness is no longer per-call overridable.
  `logged_at` is always obtained from a registry-level clock injected **only** through the
  `ExperimentRegistry` constructor (`_clock`, defaulting to `pd.Timestamp.now(tz="UTC")`).
  Rationale: determinism tests genuinely need an injectable clock (R§16.3), but a *per-record*
  override is a backdating switch. Constructor-level injection keeps the test seam while removing
  the per-call one.
- **R§21.1.4 (blocking tests).** (a) an external `capability="adapter"` / forged object raises;
  (b) a proxy dataset declared `native_or_proxy="native"` through the **real public API** raises a
  validation failure — asserted against the public path, not an internal helper;
  (c) `record_experiment` has no parameter whose name contains `recorded_via` or `logged_at`
  (introspect `inspect.signature`, so a reintroduction fails the test).

## R§21.2 Rendering defects (production defects 2 and 3 — blocking)

- **R§21.2.1.** `render_summary` MUST render the **folded** `notes` (`fe.notes`), not the immutable
  record's (`store.py:1308` reads `r.notes`). Measured: after
  `annotate(id, note="LATER ANNOTATION: this result was wrong")` the correction was in the folded
  view and absent from `summary()`. R§8.4 makes the folded view what the API returns, and R§20.11's
  entire rationale is that an unrendered note is how honesty statements get lost.
- **R§21.2.2.** `manual_results_justification` MUST be rendered (R§20.3.2 says "recorded **and**
  rendered"); it is currently recorded, passed to `_assemble_warnings` as a dead parameter, and
  printed nowhere. Render it immediately beneath the manual-path warning line. Remove the dead
  parameter or use it.

## R§21.3 Mandatory-registration allow-list (production defect 4 — blocking)

`tests/registry/test_r20_amendments.py:181` blanket-excludes any path containing
`registry_migration`, an exemption **not** granted by R§20.2.2 and not disclosed by the implementer.
Measured: a rogue driver at `experiments/_audit_rogue/rogue.py` turns the test RED (correct), while
the identical file at `experiments/registry_migration/_audit_rogue2.py` leaves it GREEN.

- **R§21.3.1 (blocking).** The only permitted exemption is an explicit, **named file** allow-list
  for `experiments/qr_smoke_001/{pipeline,crossvenue,reconstruction}.py`. Directory-prefix
  exclusions are forbidden; `__pycache__` filtering is permitted (not source).
- **R§21.3.2 (blocking).** The check MUST NOT be satisfiable by a mention of `record_run` in a
  comment or string (measured GREEN under v1.2's text-containment check). Use the AST: a module
  that *calls* `run_backtest` must obtain it via the sanctioned path.
- **R§21.3.3 (blocking test).** A temporary rogue driver placed under **each** of
  `experiments/<new>/` and `experiments/registry_migration/` MUST turn the test RED; delete both
  afterwards and verify workspace integrity.

## R§21.4 The containment boundary must be tested at the production layer (blocking)

R§20.8.5 was not closed. `src/registry/backtest_adapter.py:104` `d.data_start <= frame_start`
mutated to `<` **SURVIVED**, as did `:114` `d.data_start > d.eval_start` mutated to `>=`. The
v1.2 fixture `test_R12_4_zero_warmup_boundary_data_start_equals_eval_start` routes through
`record_experiment` → `DatasetRef.__post_init__` and **never reaches
`_check_window_containment`** — the boundary was added at the wrong layer.

- **R§21.4.1 (blocking).** The replacement fixture MUST call `record_backtest_result` (or
  `_check_window_containment` directly) with a real `BacktestResult` whose
  `dataset_windows[...]["data_start"] == result.equity_curve.index[0]`, i.e. a **zero-warm-up**
  run, and assert it is ACCEPTED. Both mutations above MUST then go RED.
- **R§21.4.2.** Note why no existing fixture reaches it: real Window A has raw start
  `2026-01-20 21:00Z` vs frame start `2026-01-25 00:00Z`, strictly inside the bound.

## R§21.5 Per-field identity protection (blocking)

`DatasetRef.semantic_dict` (`src/registry/models.py:304-320`) currently permits **each** hashed
field to be deleted individually with all 245 tests green. The only covering test varies
`content_hash` **and** `eval_start` together, so it detects the conjunction alone (measured:
A40+A41 together BROKE; each alone SURVIVED).

- **R§21.5.1 (blocking).** **One test per field**, each varying exactly that field and asserting
  the appropriate identity semantics: `native_or_proxy`, `proxy_for`, `processing_version`,
  `dataset_version`, `data_start`, `data_end`, `eval_start`, `eval_end`, `content_hash`,
  `content_hash_method`, `dataset_id`, `source_venue`, `field_type`, `symbols`. A test that changes
  two or more simultaneously does not satisfy this clause for either.
- **R§21.5.2.** Assert the correct hash per field: all of the above change `semantic_hash` and
  `exact_hash`; `content_hash`, `content_hash_method`, `data_start`, `data_end`, `eval_start`,
  `eval_end` MUST **not** change `config_family_hash` (R§20.5.4); the remainder MUST change it.
  This makes the family-vs-semantic boundary itself regression-protected.
- **R§21.5.3 (blocking).** Cover the payload-assembly ordering too: datasets sorted in the semantic
  payload (audit A29 — the mutant produced two different hashes for the same two datasets supplied
  in reversed order, i.e. duplicate detection silently lost for any multi-dataset record).

## R§21.6 Remaining known survivors to close (blocking; this list is CLOSED)

Add exactly one discriminating test per item. **Do not invent new mutation classes to inflate a
count** — the objective is regression protection of known invariants.

| audit id | behaviour left unprotected | location |
|---|---|---|
| A11 | `survivorship_safe` caller/result mismatch accepted | `backtest_adapter.py:188` |
| A14 | `contract_versions` key-set check removed | `models.py:170` |
| A19 | record-level `DIRTY_WORKTREE` dropped | `store.py:380` |
| A20a | `MISSING_ARTIFACT` dropped from `_assemble_warnings` (fold masked it in every API return — so the **on-disk record** must be asserted, not only the folded view) | `store.py:396` |
| A28 | record-level warnings not sorted — **requires a cross-process test**: `tuple(set(...))` order differs across `PYTHONHASHSEED`, so a fixture writing both records in one process cannot see it (R§10.3/R§16.2 byte-identity) | `store.py:463` |
| A29 | datasets not sorted in semantic payload | `store.py:617` |
| A38 | `PREFIX_COLLISION` finding removed | `store.py:1102` |
| A40–A44 | per-field identity — superseded by R§21.5 | `models.py:304` |
| M27b/c | adapter containment bounds — superseded by R§21.4 | `backtest_adapter.py:104,114` |
| N15 | `near_duplicates()` returns singleton groups (group-size ≥ 2 contract) | `store.py:855` |
| N17 | OOS overlap skips same-`(strategy, search_space_id)` records — an explicitly blocking clause of R§20.6.1 with no test | `store.py:423` |
| N30 | `descendants_of` unsorted (its current test is non-discriminating: two sibling children are already in sorted order) | `store.py:819` |
| N33 | `_extra_warnings` allow-list removed (no test feeds a bad token) | `store.py:368` |
| H1 | `search_space_id` filter always matches | `store.py:984` |
| H2 | `config_family_hash` filter always matches | `store.py:986` |

**R§21.6.1 Confirmed VACUOUS — no action, recorded so they are not re-litigated:** N19 (git-tracked
check removal is redundant: `rev-parse HEAD:<path>` fails anyway for an untracked file), K4
(`tz_convert("UTC")` removal — pandas stores tz-aware as UTC epoch ns, buffers byte-identical), and
M25-as-first-applied (folded-status branch alone is masked by the `results is None` branch; the full
collapse M25b is discriminating and already BROKE).

**R§21.6.2 Also fix these v1.2 material warnings while in the same code:** `ArtifactRef.__init__`
remains fully permissive (an arbitrary `sha256` for an arbitrary path is constructible) — make
`from_file` the only way to obtain a hash-bearing ref, or validate in `__post_init__`; and retire
`tests/registry/test_artifacts.py`'s in-test helper, which still reads as coverage it is not.

## R§21.7 `n_configs_evaluated`: UNKNOWN must be distinguishable from a verified 1 (blocking)

`store.py:540` defaults it to `1`, so omission silently asserts "exactly one configuration was
evaluated" — the registry inventing multiple-testing information it was never given.

- **R§21.7.1 (blocking).** The parameter becomes **REQUIRED with no default**, keyword-only, and
  tri-state: an `int >= 1` (a verified count) or `None` (**UNKNOWN**). `0` and negatives raise
  `ValidationError`.
  **Correction (v1.3, post-audit):** this clause originally also said "omission raises
  `ValidationError`", which is self-contradictory — a keyword-only parameter with no default raises
  `TypeError` at call time, and the two halves cannot both hold in Python. `TypeError` is the
  stronger guarantee (the call cannot even be made), so **omission raises `TypeError`** and that is
  the correct behaviour. Recorded rather than silently reinterpreted.
- **R§21.7.2 (blocking).** `None` renders as `n_configs_evaluated: UNKNOWN` and MUST render
  **always** — unlike a verified count, which R§20.11 renders only when `> 1`. A verified value
  renders as `n_configs_evaluated: <n> (verified)`.
- **R§21.7.3 (blocking).** Record-level warning token `N_CONFIGS_UNKNOWN` when `None`. New filter
  `n_configs_unknown: bool`.
- **R§21.7.4 (blocking).** `search_space_summary` MUST NOT silently sum over `None`. It reports
  `n_configs_evaluated_total` (sum of known values), `n_records_with_unknown_n_configs`, and
  `n_configs_evaluated_total_is_lower_bound: bool` (True when any member is UNKNOWN). Rationale: a
  total that quietly skips unknowns understates the multiple-testing denominator, which is the one
  number this field exists to protect.
- **R§21.7.5.** Unhashed, as in v1.2 — it is metadata about the search, not about the
  configuration, so this change causes no identity churn. Every existing caller and test must be
  updated to pass it explicitly; the R§17 migration passes `n_configs_evaluated=1` (each smoke
  window is genuinely one configuration).

## R§21.8 Invalidation history through the public read path (blocking verification)

Already implemented in v1.2 and confirmed working; R§21 requires it be **explicitly re-verified via
the normal public read path**, because it is the defect most likely to regress silently:
`set_status(INVALID, …)` then `set_status(COMPLETED, …)` MUST leave `WAS_INVALIDATED` in the folded
record-level warnings (irremovable), MUST leave the full `status_history`, MUST render both in
`summary()`, and MUST be matchable by `ever_status="INVALID"` — all read through
`load_experiment`/`find_experiments`/`summary`, not internal helpers.

## R§21.9 `config_family_hash` excludes `frozen_spec_sha256` (legitimised)

The implementer stripped `frozen_spec_sha256` from `config_family_hash` beyond R§20.5.4's literal
7-field list, arguing `OOS_RELABEL_OF` would otherwise be unreachable. **Adopted into the spec** —
a deviation from a frozen normative list must not live in a code comment. The auditor's correction
is recorded: "permanently unreachable" was too strong; retaining the field would leave the check
firing only where an earlier non-OOS run declared the identical `frozen_spec_ref`, i.e. unreachable
in every realistic case. `semantic_hash` retains the field, so exact identity loses nothing.
R§20.5.4's strip-list is amended to 8 entries: `content_hash`, `content_hash_method`, `data_start`,
`data_end`, `eval_start`, `eval_end`, `recorded_via`, `frozen_spec_sha256`.

## R§21.10 Explicitly NOT in scope (deferred; the registry MUST NOT claim otherwise)

Documented so no future reader mistakes the registry's guarantees for more than they are. These
belong to the upcoming research-methodology / protected-OOS layer:

| # | deferred item |
|---|---|
| D13 / DEFERRED-005 | config files outside `CODE_SCOPE_PATTERNS` — MUST close before the first config-file-driven strategy is registered |
| D17 | **Unrecorded experiments cannot be prevented by the registry.** Hiding a run requires only not calling the API. `run_and_register` and `record_run` are mitigations, not enforcement; mandatory workflow integration is a prerequisite for alpha research, not part of this closure |
| D18 | **A misleading `search_space_id` cannot be prevented.** Grouping is only as honest as the label; `n_configs_evaluated` is self-reported (D14) |
| D19 | **OOS contamination is not fully enforced.** R§20.6.1 checks ancestors and same-`(strategy, search_space_id)` records, but an OOS record declaring a different `search_space_id` **and** an unrelated parent evades the overlap warning (measured). Only `OOS_RELABEL_OF` catches the identical-config variant. Real enforcement requires the protected-OOS layer |

**R§21.10.1 (blocking documentation requirement).** `experiments/registry/README.md` MUST state
plainly that the registry records what it is told and does not prevent unrecorded experiments or all
OOS misuse, citing D17–D19. A tool that overstates its guarantees is worse than one that states
them.

## R§21.11 Verification requirements

- Re-run the R§18.2 M-table and the audit's A/N/H survivors listed in R§21.6. **Zero survivors** is
  the acceptance criterion. Do **not** expand into an open-ended mutation search.
- Per-target-test mutation runs only (watchdog risk).
- Workspace integrity per R§20.8.10: hash manifest **paired with a file-set comparison**, verified
  after every mutation, covering tracked AND untracked files.
- No test may write into the real `experiments/registry/` (R§20.8.2), including from a child
  subprocess.
