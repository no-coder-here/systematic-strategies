# Data Layer Contract — SPECIFICATION v1.4 (QR-DATA-001)
...
| v1.4 | 2026-08-16 | **USER DECISION — DEFER EVENT_PRICE FUNDING.** Supersedes v1.3's DECISION 4 authorization: the historical `asset_ctxs` extraction is **NOT to be run for now**. `funding_notional_basis="period_start"` is the operative default for all research. `event_price` remains architecturally supported but **never live-backfilled or validated**, and is an optional future enhancement, not a blocker. See D§5.5.1 rule 2 (revised) and `docs/TODO.md`. |
| v1.3 | 2026-08-16 | **USER DECISIONS 1–5.** Recorded, not re-audited (no new spec round required). Confirms D§16.7 single-source (1), D§16.3.3 renames-unmapped (2), D§14.0 trade archive still forbidden (5). **Changes**: D§16.3.4 + D§17 — an *explicitly verified* unit multiplier is now permitted and cross-venue comparison happens **after** unit normalization; only an *unexplained* order-of-magnitude gap fails (3). **Authorizes** D§5.5.1 `asset_ctxs` compact streaming extraction, with incremental updates (4). |
...
2. **DEFERRED (v1.4) — do NOT run.** v1.3's DECISION 4 authorization is **withdrawn for now**. The
   compact streaming extraction path is implemented and unit-tested, but MUST NOT be executed
   against the real archive without a fresh, explicit authorization. Consequently:

   - `funding_notional_basis = "period_start"` is the **operative default** for all research and
     backtests, and MUST be stated explicitly in research outputs (its error bound is §7.6's
     `|rate| × max intra-period move`; −6.98% on a +15% intra-day move).
   - **`event_price` provenance MUST NOT be claimed unless the oracle dataset actually exists.**
     A missing compact artifact MUST leave `notional_price` NaN and the basis `period_start` — it
     MUST NOT be silently treated as `event_price`, and no placeholder artifact may be fabricated.
   - The pipeline below remains normative for **if and when** the extraction is authorized.

   The pipeline is strictly incremental, one archive segment at a time:
   ...
   Peak local raw footprint MUST stay at roughly one segment, never the ~9.6 GB archive.
...
6. **Incremental refresh (DECISION 4).** After the initial backfill, updates MUST fetch and process
   **only new archive segments**. Re-downloading full history on refresh is a defect. A persisted
   high-water mark (last processed archive date + row counts) MUST drive this, and a refresh over
   an already-processed range MUST be a no-op rather than a duplicate append.
7. **Reporting and alignment validation (DECISION 4).** The final **processed size** MUST be
   reported, and the **oracle/funding alignment MUST be validated**: every funding event that
   should carry a price has one, each price comes from the correct containing minute
   (`ctx_minute <= event_timestamp`, nearest preceding, gap ≤ 2 min), and no event is silently
   left unpriced. Report the count of events priced, unpriced, and the distribution of the
   event→ctx time offset.
...
| M36 | Drop the recorded unit multiplier so normalized levels differ ~1000× (D§16.3.4) | unit-normalization test (**must fail, not warn**) |
| M42 | Accept a mapping whose unit multiplier is absent or unverified (D§16.3.4) | unverified-multiplier test (**must FAIL the mapping**) |
| M43 | Infer the unit multiplier from the observed price ratio (D§16.3.4) | circular-inference test |
| M44 | Compare cross-venue prices **before** unit normalization (D§17.2) | normalize-then-compare test |
| M45 | Infer a rename/migration from symbol similarity (D§16.3.3) | no-heuristic-lineage test |
| M46 | Re-download full `asset_ctxs` history on refresh (D§5.5.1.6) | incremental-refresh test |
| M47 | Retain raw `asset_ctxs` segments after extraction (D§5.5.1.2) | peak-footprint test |
| M48 | Silently leave a funding event unpriced past the 2-min gap (D§5.5.1.7) | alignment-validation test |
| M49 | Concatenate the Binance and Hyperliquid canonical datasets (D§16.7 DECISION 1) | no-merge test |
...
3. **Rename chains stay unmapped (DECISION 2, confirmed).** `MATIC`/`POL`, `RNDR`/`RENDER` and
   similar remain **unmatched for QR-DATA-001** unless an *explicit verified lineage mapping*
   already exists — and none does at freeze. **Migrations MUST NOT be inferred from symbol
   similarity**, string distance, or any heuristic. A reviewed instrument-lineage mapping layer
   will be added later; until then these assets carry native-only history.
4. **Unit-scaled contracts (DECISION 3, revised in v1.3).** The mapping layer MUST support an
   **explicit unit multiplier** per mapping entry:

       {hl_symbol, venue_symbol, hl_unit_multiplier, venue_unit_multiplier, verified_by, status}

   - An **explicitly verified** multiplier is **permitted** — `kPEPE` (1000 tokens) ↔
     `1000PEPEUSDT` (1000 tokens) is a legitimate mapping once both multipliers are recorded and
     verified, not merely pattern-matched from the name.
   - The multiplier MUST come from venue metadata / documentation and be recorded with its
     `verified_by` evidence. **It MUST NOT be inferred from observed price ratios** — inferring
     the multiplier from the data would make the D§17 check circular, since the check exists to
     detect a wrong multiplier.
   - **Neither venue exposes a machine-readable multiplier** (verified 2026-08-16): Binance
     `1000PEPEUSDT` carries only `baseAsset: "1000PEPE"` — a *name* convention — and Hyperliquid's
     `kPEPE` entry is `{szDecimals: 0, maxLeverage: 10, marginTableId: 52}` with no unit field at
     all. "Explicitly verified" therefore means a **checked-in, human-reviewed constant carrying a
     documentation citation** in `verified_by`, not a field read at runtime. Parsing the multiplier
     out of the symbol string at runtime is **not** verification and MUST fail the mapping — that
     is exactly the heuristic D§16.3 forbids.
   - **If the required multiplier is not explicit and verified, the mapping FAILS.** It is not
     downgraded to a warning and the pair is not silently used at raw scale.
   - An **unexplained** order-of-magnitude discrepancy surviving normalization MUST **FAIL**
     (D§17.4). An explained, verified multiplier is not a discrepancy at all.

5. **Normalize before comparing.** All cross-venue price comparison (D§17) operates on series
   already normalized to a **common underlying unit** via the recorded multipliers. Comparing raw
   quoted prices across venues with different contract units is a category error.
...
**DECISION 1 (approved, v1.3).** Two datasets are maintained **side by side, never merged**:

| dataset | role |
|---|---|
| Binance USDⓈ-M 1h (matched symbols) | canonical long-history research price series |
| Hyperliquid-native OHLCV | native venue-validation data, retained separately |

Research must be able to run **long Binance-proxy backtests** and **overlapping
Hyperliquid-native validation** of the same strategy without either path creating an artificial
venue seam inside one canonical series. Neither dataset is derived from the other, and a loader
MUST NOT silently concatenate them.
...
**Precondition (DECISION 3):** both series MUST first be normalized to a common underlying unit
using the explicit verified multipliers of D§16.3.4. Every metric below is computed on
unit-normalized series. Return-based metrics are multiplier-invariant; the **level** comparisons
are not, and are meaningless without this step.
...
An order-of-magnitude level discrepancy **surviving unit normalization** (D§16.3.4, D§17.2) is
**not** a proxy finding — it is a mapping defect and MUST be raised as such, and MUST fail. A
discrepancy fully explained by a recorded, verified multiplier is not a discrepancy: it is the
multiplier doing its job, and MUST NOT be reported as a proxy-quality problem.
...
7. **22 Hyperliquid assets have no Binance proxy** (F14) and are limited to native history.
   **DECISION 2**: rename cases stay unmapped; a reviewed **instrument-lineage mapping layer** is
   deferred to a later work order and is the tracked follow-up.
8. **`asset_ctxs` extraction DEFERRED (v1.4).** Architecturally supported, implemented, unit-tested
   — but never live-backfilled or validated. `period_start` is the operative basis. Tracked as an
   optional future enhancement in `docs/TODO.md`, not a blocker for the research pipeline.
